...
<br><br><br>Linux、UNIX、または Windows で JVM からスレッドダンプを取得する方法を教えてください。<br><br><br><br> <br><br>
...
<br><br>手順 1:Java プロセスの PID を取得します。<br><br><br><br> <br><br>
...
<b>注意：</b> Linux および UNIX では、sudo -u user jps -l というコマンドを実行する必要がある場合があります。ここで、「user」は、Java プロセスを実行しているユーザーのユーザー名です。

これが動作しない場合や、まだ Java プロセスが見つからない場合（パスが設定されていない、JDK がインストールされていない、または古い Java バージョン）は、

- UNIX、Linux、Mac OS X:ps -el | grep java
...
コンソール出力 redirect/append ディレクティブを使用して、連続するスレッドダンプをファイルに出力できます。jstack -l pid threaddumps.log
...
- jstack コマンド (jstack -l pid threaddumps.log) が以下のエラー 1 をスローした場合は、java プロセスを所有するユーザーとしてコマンドを実行します。  例：
...
次のように実行します。sudo -u *ユーザー* jstackSeries.sh *pid* *aemserveruser* *count* *delay*
...
1. 次のコマンドを実行して、これを実行します。
   ...
   sudo -u user kill -QUIT と同様に、このコマンドを実行する必要がある場合があります。 *pid* ここで、「user」は、Java プロセスを実行しているユーザーです。
2. crx-quickstart/server/start スクリプトを使用して CQSE を起動する場合、スレッドダンプはcrx-quickstart/server/logs/startup.logに出力されます。 JBoss、WebSphere、Tomcat などのサードパーティのアプリケーションサーバーを使用している場合は、サーバーのドキュメントを参照して、標準出力が送られるファイルを確認してください。
...
1. Communique を実行する jvm に次のパラメーターを追加します。 -Dcom.sun.management.jmxremote
