...
このエラーは、npm を使用して失敗したモジュールのノードバージョンを更新することで解決する必要がある、顧客コードのエラーです。 この変更をおこなうには、次の手順を実行する必要があります。

1. reactor pom.xml で使用されているノードのバージョンを探します。 次の例= 16.14.0    `pom.xml`:
   ...
   configuration
       ...
       configuration
   ```
2. このノードのバージョンを次の形式で追加 <b>devDependencies</b> 内 `package.json` ビルドに失敗したモジュールのファイル。 上記の例では、UI フロントエンドです。    前 — `package.json`:
