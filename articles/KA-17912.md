---
title: キャッシュ・ディレクトリについて
description: 「Adobe Experience Managerでの Dispatcher のキャッシュの発生方法と設定方法を説明します。」
...
<b>問題/症状</b>
このドキュメントでは、Dispatcher のキャッシュの発生方法と設定方法について説明します。
[目次](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17490.html)
...
キャッシュ時にエクスペリエンスを壊す要素がある場合は、ルールを追加して、その項目をキャッシュするオプションを削除できます。 上の例で示すように、 csrf トークンはキャッシュされず、除外されているはずです。 これらのルールの作成に関する詳細は、こちらを参照してください。 [ここ](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=ja#configuring-the-dispatcher-cache-cache).
