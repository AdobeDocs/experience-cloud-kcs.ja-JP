---
title: 「キャッシュについて」 | AEM"
description: 「Dispatcher のキャッシュが発生する方法と、Adobe Experience Managerでの Dispatcher のキャッシュの設定方法について説明します。」
solution: Experience Manager
product: Experience Manager
applies-to: "Experience Manager"
keywords: "KCS，キャッシュ， Dispatcher キャッシュ， AEM,Experience Manager"
...
この記事では、Dispatcher のキャッシュがどのようにおこなわれるか、およびベースラインインストールで以下のデフォルトのキャッシュディレクトリを使用して Dispatcher のキャッシュを設定する方法について説明します。
...
パブリッシャーレンダラーにソケットタイムアウトまたは 500 エラーメッセージがある場合、この機能はトリガーしません。 AEMに単に到達できない場合、この機能は何もしません。
...
それでも、AEMが TTL ヘッダーを送信するように設定して、Dispatcher がヘッダーを遵守できるようにする必要があります。 この機能を切り替えると、AEMがキャッシュ制御ヘッダーを送信したファイルを削除するタイミングを Dispatcher が把握できるようになります。 AEMが TTL ヘッダーの送信を開始しない場合、Dispatcher はここで特別な処理をおこないません。
