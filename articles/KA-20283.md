---
title: 「Mixin の競合（および ClassicUI の使用）が原因でロールアウトが失敗した」
description: 「（ClassicUI を使用した）mixin の競合が原因でロールアウトが失敗する問題を修正する方法を説明します。」
...
（ClassicUI を使用した）mixin とコンテンツ更新アクションの設定が競合しているので、ロールアウトが失敗する問題を修正する方法について説明します。
...
2. に移動します。 <b>/siteadmin</b> ( その場合、 <b>クラシック UI</b>)
3. ページプロパティを開く */content/we-retail/language-master/en/women*
4. 画像内 *タブ* 任意の画像をサムネールとして設定
5. ページをロールアウト
6. 次に移動： <b>サイト UI</b>
7. ページプロパティを開く */content/we-retail/us/en/women* 再び
8. （内） *サムネール* 「 」タブで、「 」をクリックします。 *プレビューを生成* 変更を保存します。
9. /siteadmin でページを再度ロールアウトします。
...
この問題は、AEMがライブコピーページの jcr:content (/content/we-retail/us/en/women/jcr:content/image/file/jcr:content) の下に「dam:thumbnails」ノードを作成できず、ロールアウト時に「dam:thumbnails」がライブコピーページノード&quot;/cont/we-we-re/we-retail/en/jcr/jcr:content/image/file/jcr:content フォルダーと「dam:Thumbnails」フォルダーは、「dam:Thumbnails」ノードタイプの下にのみ作成できます。

MSM のデフォルトの動作では、ロールアウト時にソースのノードに追加された新しい mixin が無視されます。 `[` 0`]`  したがって、これは「/content/we-retail/us/en/women/jcr:content/image/file/jcr:content」に追加されません。
