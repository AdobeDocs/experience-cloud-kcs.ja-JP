...
<br><br>注意：このガイドは、Experience CloudID サービス拡張機能が既にに実装されていることを前提としています。 [!DNL Launch].<br><br>
...
<br><br>手順 2<br><br>
次に、お客様から同意を得るメカニズムを構築します。 これは、 [!DNL Javascript] または任意のサーバー側スクリプトを使用できます。 Cookie の同意ダイアログボックスをユーザーに提示し、選択を記録する必要があります。
...
上記のコードが実行されると、 [!DNL Analytics] サーバー呼び出しが実行されます。
