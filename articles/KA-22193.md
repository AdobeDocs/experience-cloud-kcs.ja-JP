---
title: 「リクエストヘッダーフィールドのサイズがサーバーの制限を超えています」
description: 「AEMでページを開く際に\「無効なリクエスト\」エラーを解決する方法を説明します。」
solution: Experience Manager as a Cloud Service,Experience Manager
product: Experience Manager as a Cloud Service,Experience Manager
applies-to: "Experience Manager as a Cloud Service,Experience Manager"
keywords: "KCS, サイズ，リクエストヘッダー，超過，サーバー制限，AEM, Adobe Experience Manager, トラブルシューティング，AEMaaCS, Adobe Experience Manager as a Cloud Service"
...
ページを開くと、「<b>リクエストが正しくありません</b>「」という応答とメッセージ：

“<b>*リクエストヘッダーフィールドのサイズがサーバーの制限を超えています</b>*“
...
Apache has a default `LimitRequestLine` of *8K*. 詳しくは、次を参照してください [Apache の主な機能：LimitRequestLine ディレクティブ](https://httpd.apache.org/docs/2.4/en/mod/core.html#limitrequestline) Apache web サイトで、次の操作を行います。

このエラーは、の値を増やすことで解決できます。 `#LimitRequestFieldSize` および `#LimitRequestLine`.

<u>AMS/オンプレミスの場合</u>:

ヘッダー制限は Apache 設定ファイルで増やすことができます。 `httpd.conf`.



<u>Aemaacs の場合</u>:

Vhost ファイルに上記のプロパティを追加して、リクエストヘッダーの上限を増やします。
