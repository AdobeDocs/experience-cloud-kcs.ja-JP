...
また、ACL は結果に適用されないので、現在のセッションに表示されないノードも、返されるカウントに含まれます。 したがって、返されるカウントは実際の結果数よりも大きくなり、正確なカウントは、結果を繰り返し処理することによってのみ決定できます。
   ...
   - <b>Lucene インデックスの設定</b>\
     ...
     詳しくは、 [https://jackrabbit.apache.org/oak/docs/query/lucene.html](https://jackrabbit.apache.org/oak/docs/query/lucene.html) 使用可能なパラメータの詳細一部のパスはインデックスを作成する必要がないので、次の操作を行うことができます。CRXDE Liteで、/oak:index/lucene に移動し、次の値/var、/etc/workflow/instances、/etc/replication を持つ excludedPaths という複数値の文字列プロパティ (String) を設定します。
   - <b>データストア</b>\
