...
keywords: 「KCS、SQL パフォーマンス・チューニング、ETL システム」
...
6. 十分な RAM をプロビジョニングし、IO の飽和を回避\
   ...
   - データディスク上のリードアヘッドバッファを 32 に設定します。
...
7. Linux での Transparent Huge Pages の無効化\
   AEMは詳細な読み取り/書き込みを実行しますが、Linux の Transparent Huge Pages は大規模な操作に最適化されているので、 [透明な巨大ページを無効にする](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/introduction/aem-with-mongodb.html?lang=en#operating-system-support) Mongo または Tar ストレージを使用する場合。
8. 一時的なワークフローの有効化\
...
Adobeでは、CPU コアの 50%を超えないようにすることをお勧めします。 この値を調整するには、次の手順に従います。https:/host:port/system/console/configMgr/org.apache.sling.event.jobs.QueueConfiguration設定 `queue.maxparallel` を、AEMインスタンスをホストするサーバーの CPU コアの 50%を表す値に設定します。 例えば、8 個の CPU コアの場合、の値を 4 に設定します。
10. Oak リポジトリの調整\
...
また、ACL は結果に適用されないので、現在のセッションに表示されないノードも、返されるカウントに含まれます。 したがって、返されるカウントは実際の結果数よりも大きくなり、正確なカウントは、結果を繰り返し処理することによってのみ決定できます。
     ...
     注意：有効化 *fastQuerySize* を使用すると、クエリの応答が高速化されます。 ただし、AEMは一部のクエリに対して不正確な結果数を返します。 アプリケーションで正確な結果数を使用する場合は、 *fastQuerySize*.
...
多数のバイナリを処理する場合、パフォーマンスを最大化する必要があります。 したがって、Adobeでは、デフォルトのノードストアの代わりに外部データストアを使用することをお勧めします。 また、Adobeでは、次のパラメーターを調整することをお勧めします。
     ...
     注意：この *cacheSizeInMB* 設定が大きすぎると、Java プロセスのメモリが不足する可能性があります。 例えば、最大ヒープサイズを 8 GB(-Xmx8g) に設定し、AEMとアプリケーションが 4 GB の結合ヒープを利用すると想定する場合、cacheSizeInMB を 164 ではなく 82 に設定すると効果的です。 最大ヒープの 2～10%の範囲では、安全な設定です。 ただし、Adobeでは、メモリ使用率も監視しながら、負荷テストで設定の変更を検証することをお勧めします。
11. Mongo ストレージの調整
   ...
   - <b>Oak 1.4.x を使用したAEM 6.2 のキャッシュ設定：</b>
      - AEM 6.2 では、これらのキャッシュ設定の動作方法が変更されました。 Oak 1.4 を使用したAEM 6.2 には、新しいキャッシュがあります。prevDocCache。 このキャッシュは、prevDocCachePercentage 設定を使用して設定できます。 初期設定は 4 です。
   ...
   - <b>MongoDB 実稼動チェックリストを実装します。</b>\
     [https://docs.mongodb.org/manual/administration/production-checklist/](http://docs.mongodb.org/manual/administration/production-checklist/) - Mongo DB のサポートによると、多くの項目がパフォーマンスに大きな影響を与えます。 ご質問は、MongoDB サポートに直接お問い合わせください。
...
このパラメータは、セカンダリから読み取りを行うようにシステムに指示し、追加の読み取りパフォーマンスを提供します。
   - <b>oak-observation のスレッドプールを増やします。</b> open */system/console/configMgr/org.apache.sling.commons.threads.impl.DefaultThreadPool.factory*        名前を oak-observation に設定し、最小および最大プールサイズを 20 に設定します。
   - <b>監視キューの長さを増やす：</b> という名前のファイルを作成します。 *com.adobe.granite.repository.impl.SlingRepositoryManager.cfg* 含まれるパラメーター *oak.observation.queue-length=50000*. 次の場所に配置します。 */crx—quickstart/install* フォルダー。
   - 長時間実行されるクエリの回避：JVM パラメーターでシステムプロパティを設定します。 *-Doak.mongo.maxQueryTimeMS=60000* 1 分を超えるクエリの実行を避けるため。
12. Tar ストレージの調整\
   ...
   別のオプションとして、 *tarmk.mode=32* in [SegmentNodeStoreService.config](https://experienceleague.adobe.com/docs/experience-manager-64/deploying/deploying/data-store-config.html?lang=ja) オペレーティングシステムの問題が解決されるまで。 無効化の欠点により、I/O が集中的に使用されます。 I/O ページのロック制限を必ず上げてください。
...
14. Adobe Managed Services(AMS) のお客様向け Cloud Manager\
