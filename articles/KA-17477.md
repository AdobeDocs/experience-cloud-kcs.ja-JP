...
![apache ベースラインインクルード順序。 apache バイナリは httpd.conf で始まり、conf.d/に対して includeoptional を実行します。*.conf と conf.modules.d/*.conf ディレクトリ](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/Apache-Webserver-Baseline-Includes.png)
...
![ams 仮想ホストにはサブ項目が含まれます。 この図は、1 つの.vhost ファイルが、変数、許可リスト、書き換えフォルダーのファイルをどのようにインクルードするかを示しています](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/Apache-Webserver-AMS-Vhost-Includes.png)
...
![<FILENAME>_farms.any は、ファームの設定を完了するためにサブ.any ファイルを含みます。 この図で、ファームに各トップレベルセクションファイルのキャッシュ、クライアントヘッダー、フィルター、レンダリングおよび vhosts .any ファイルが含まれていることがわかります](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/Apache-Webserver-AMS-Farm-Includes.png)
