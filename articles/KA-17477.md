---
title: 設定ファイルの説明 | AEM
description: Managed Servicesの Dispatcher サーバーをAdobeする際に設定ファイルを移動し、理解する方法を説明します。 目的、命名規則および使用方法を調べます。
...
これらの命名規則を使用すると、属していない自動インクルードディレクトリにファイルが移動された場合、問題を回避できます。
