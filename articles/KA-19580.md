...
1.に示す手順に従ってAEM Desktop App をインストールして設定した後、AEMオーサーインスタンスに接続すると、アプリケーションがエラー「*アプリケーションで不明なエラーが発生しました* r&quot;.<br><br>
...
この問題は、手順 8 の 1 で URL を使用すると再現できます。 `http://aem_server:port/assets.html/content/dam` は、の代わりに、Experience Managerリポジトリとの接続に使用されます `http://aem_server:port`.
