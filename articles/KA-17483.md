...
<br><br><br> <br><br>
AMS がシステムをプロビジョニングする際に、すべてが機能し、安全になるベースライン設定を展開します。  これらは、AMS が機能とセキュリティの基準として常に保証したいと思っているものではありません。  これを実現するために、一部のファイルは読み取り専用としてマークされ、変更を避けるために不変です。
...
Apache Web サーバーは、ドメインがトラフィックを生成するものであることを認識していますが、Dispatcher モジュールに対して、受け入れる新しいドメイン名を持つことを通知する必要があります。  まず、新しい\*_vhost.any ファイル/etc/httpd/conf.dispatcher.d/vhosts/weretail_vhosts.anyを作成し、そのファイル内に、受け入れたいドメイン名を入れます。
