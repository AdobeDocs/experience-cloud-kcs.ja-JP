...
description: AEMで読み取り専用および不変とマークされるファイルの処理方法を説明します。
...
既存のをコピーしました `aem_publish.vhost` ファイルを作成する必要があるものが既に含まれており、既に強力なスタートを切りたくないからです。  次に、新しい weretail.vhost ファイルを編集し、必要な変更を加えます。
...
必要な部分のみを変更し、ベースライン設定ファイルに付属の既存のインクルード指示とコードを活用していることに注意してください。  詳しく記述する必要があるのは、変更が必要な要素だけです。  作業がはるかに容易になり、メンテナンス対象のコードを減らすことができるようになりました。
