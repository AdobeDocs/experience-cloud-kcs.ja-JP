...
9. ページ全体で読み込まれる CSS で、HTMLヘッド内ではなく
...
2. AEM java プロセスが CPU 使用率を高く引き起こしている場合は、OS レベルで確認します。    
...
4.システムメンテナンス手順を確認します。 参照 [記事](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/monitoring-and-maintaining.html)AEMのメンテナンスの詳細。また、AEMで適切なメンテナンスをおこなうには、以下をおこないます。 — リビジョンのクリーンアップ（MongoMK および Database DocumentNodeStore のみ） — 日次または日次以上頻度 — オフライン Tar 圧縮（TarMK のみ） — 隔週 — データストアガベージコレクション（FileDataStore または S3 DataStore を使用するシステム） — 週次 — ワークフローのパージ — 週次 — バージョン purge - weekly - AuditLog Purge - weekly 5. AEM Dispatcher レベルで実装されているキャッシュ戦略を確認します。
6.サイトのキャッシュを確認します。
7. <b>監査</b> Google Chrome ブラウザーの機能 <b>開発者ツール</b> パネル。  これらのツールを使用すると、クライアント側のパフォーマンスの改善に関する推奨事項が表示されます。
...
<br> AEM Assetsのパフォーマンスに関する問題<br> 
...
ヒープダンプファイルを取り込んだら、次の場所で開きます。 [Eclipse マット](http://www.eclipse.org/mat/) または [IBM Memory Analyzer](https://developer.ibm.com/languages/java/) ツール  Eclipse MAT で、Leak Suptes レポートを実行し、「Thread Details」ビューを開いて、メモリ問題の潜在的な原因を確認します。
...
1. AEMインスタンスでこれらの URL を開いて、非同期インデクサーhttp://aemhost:port/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3Dasync%2Ctype%3DIndexStats http://aemhost:port/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3Dfulltext-async%2Ctype%3DIndexStats  - Thisに関する統計を表示します (URL はAEM 6.2 以降にのみ適用されます )
...
1. レプリケーションキューを確認 [ステータス](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/replication.html?lang=ja):
   ...
   <b>ブロック済み：</b> （項目がキューに入っているが処理できない場合）例えば、エージェントがダウンしているか存在しないホストを指す場合などです。
...
3. 次の場所にあるレプリケーションエージェントログを確認します。 <b>http://host:port/etc/replication/agents.author/AgentName.log.html#end</b>.  項目を特定できない場合は、このログを収集し、AEMサポートに提示します。
...
- インスタンスが動きません <b>起動中</b> 状態。
- ログファイルまたは圧縮コマンド出力レポート <b>SegmentNotFoundException</b>.
