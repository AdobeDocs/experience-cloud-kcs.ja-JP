---
title: 「技術実装ガイド：Audience Managerとキャンペーン»
description: 説明
...
このテクニカル統合ガイドは、Adobeコンサルタント、Adobeパートナー、その他の経験豊富な実装担当者を対象としています。
...
3. AAM UI でのデータソース、宛先、AAM FTP の作成/プロビジョニングは、Adobe担当者がおこなう必要があります。
...
3. を `CRM_ID` ページで使用可能顧客がログインしている必要があります。
...
4. DTM が `CRM_ID`
設定が次の中にあります <b>marketing cloud ID サービス</b> ツール

この `hashed_email` データ要素は、DTM で `CRM_ID` dataLayer で使用できます。
...
2 つのデータソースがAdobe Campaign UI で作成されます。1 つは宣言済み ID またはハッシュ化された顧客 ID に基づき、もう 1 つは marketing cloud id または要求者 UUID に基づきます。

各 ID に対応する宛先 ID は、各フィールドに追加されます。

例：AAMの宛先Adobe Campaign — 宣言済み ID の宛先 ID が、ハッシュ化された顧客 ID として紐付けキーと共に、データソース設定に追加されます。
...
の下 <b>管理</b> キャンペーン UI 内のメニューオプションで、「 <b>実稼動</b><b>テクニカルワークフロー</b>.
...
セグメントを作成すると、それらのセグメントはAAM UI の下に自動的に表示されます。 <b>特性</b><b>すべての特性</b><b>Adobe Campaign Traits</b>.
...
- 次を持っていません： <b>共有オーディエンスを作成</b> のオプション <b>リスト</b>理由は何でしょう？
