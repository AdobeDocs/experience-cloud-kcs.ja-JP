---
title: "Campaign へのAudience Manager"
description: 説明
...
10. ユーザーは、Adobe IDを介して Campaign V6 にログインできる
...
2. CRM データベースを Campaign 顧客と同期するには、CRM データを Campaign にプッシュするプロセスを設定する必要があります。 電子メール、 CRM_ID は、同期される最小フィールドです。
...
4. DTM は、「marketing cloud ID サービス」ツールで CRM_ID 設定を読み取ります。 hashed_email データ要素は、dataLayer で使用可能な CRM_ID を読み取る DTM で設定されます。
...
AAM用に作成した SFTP は、Campaign がAAMからセグメントをプッシュおよびプルできるように、Adobe Campaignで設定する必要があります。 SFTP 資格情報は、AdobeカスタマーケアまたはAdobe担当者から提供されます。 プロビジョニング中には、AAMにオーディエンスを送信し、AAMから campaign にセグメントをオンボーディングするために campaign で使用される、ftp-in と ftp-out の両方の場所がプロビジョニングされます。
...
7. &#x200B;&#x200B;&#x200B; &#x200B; &#x200B;   Adobe Campaign内のデータソースを設定するデータソースは、Adobe Campaignで設定します。 2 つのデータソースがAdobe Campaign UI で作成されます。1 つは宣言された ID またはハッシュ化された顧客 ID に基づき、もう 1 つは marketing cloud id または要求元の UUID に基づきます。 各 ID に対応する宛先 ID は、各フィールドに追加されます。 例えば、AAMの宛先Adobe Campaign — 宣言済み ID の宛先 ID が、ハッシュ化された顧客 ID として紐付けキーと共に、データソース設定に追加されます。
...
8. サイトでのユーザー行動の追跡以下の使用例では、顧客が使用する検索キーワードが evar2 に取り込まれ、Analytics での購入イベントとして商品の購入が行われます。 以下のフローは、AAMへの AA 呼び出しのサーバー側転送に基づいています。 検索ページでは、「q」パラメーターを取り込んで、データ要素に配置します。
...
その後、Analytics の「ご購入ありがとうございました」ページで「購入」イベントを取り込みます。
...
この時点で、顧客を追跡し、Audience Managerでセグメントを作成できます。 また、Audience Managerとキャンペーンの両方が同じ CRM_ID を持つユーザーを収集しています。

9. &#x200B;&#x200B; &#x200B; &#x200B; &#x200B;  AAM Create segments in  based on user behavior Then create following traits を作成します。「冬を検索済み」:
...
10. &#x200B;&#x200B; &#x200B; &#x200B; &#x200B;  AAMへのセグメントのプッシュ AMC オーディエンスをキャンペーンと共有するには、でセグメントを作成した後に、Adobe Campaign内で設定をおこなう必要があります。 まず、Adobe Campaign内で共有オーディエンスを作成し、セグメントリストから AMC セグメントを選択します。 キャンペーン内で、MID と宣言済み ID に基づいて 2 つの共有オーディエンスを作成する必要があります。 設定が完了すると、セグメントは、AAM内の各宛先に自動的にマッピングされたものと見なすことができます。
...
11. AMC 経由でAAMとキャンペーンオーディエンスを共有する AMC 経由でAAMとキャンペーンオーディエンスを共有するには、Adobe Campaign内で次の設定が必要です。 キャンペーン UI の「管理」メニューオプションで、実稼働用テクニカルワークフローを選択します。 新しいテクニカルワークフローをクリックし、クエリをドラッグして共有オーディエンスを更新してから終了します。
...
キャンペーンで、まずオーディエンスに基づいてリストを作成します。 これは「リスト」にあります。
...
ポップアップで、Marketing Cloud オーディエンスを選択できるようになりました。 「search_for_winter_did_not-buy」を選択します。
...
Adobe IDを使用してキャンペーンにログインしていない可能性があります。 Adobe IDを使用してログインした場合は、Enterprise Console UI で Audience Library 製品にMarketing Cloudされていることを確認してください。
...
3. AAM UI からキャンペーンの宛先にセグメントをマッピングしないでくださ&#x200B;い
