---
title: 「Adobeの Launch へのビデオ分析の実装」
...
「規則」タブの「規則を追加」ボタンをクリックして、Adobe起動プロパティに新しい規則を作成します。 このルールには、YouTube ビデオを読み込むためのイベント条件、ビデオ分析およびYouTube iFrame API スクリプトが含まれます。
...
イベントと条件の下の「+」ボタンをクリックして、YouTube ビデオを起動する条件を作成します。 例えば、YouTube ビデオを起動するための特定の div 「player」を検索する条件「Element Exists」を追加したとします。
...
1. 拡張機能ドロップダウンから「Core」を選択します。
2. アクションタイプ ドロップダウンから「カスタムコード」を選択します。
3. t サイズ：12.0pt ”`>` 「名前」テキストボックスにアクション名を追加します。
4. 言語の下の「Javascript」を選択します。
5. 「グローバルに実行」をオンにします。
6. 「編集画面を開く」をクリックします。
...
2. アクションタイプ ドロップダウンから「カスタムコード」を選択します。
3. 「名前」テキストボックスにアクション名を追加します。
4. font-size:12.0pt&quot;`>` 言語の下の「Javascript」を選択します。
5. pan style=&quot;font-size:12.0pt&quot;`>` 「グローバルに実行」をオンにします。
6. style=&quot;font-size:12.0pt&quot;`>` 「編集画面を開く」をクリックします。
7. span style=&quot;font-size:12.0pt&quot;`>` 訪問者 ID サービスコードの追加
