...
お客様は、AEMヘッドレスソリューションを使用してAEMインスタンスからデータを取得する必要がある Web サイトを開発中です。

エンドポイントを設定するには、 `[` 1`]`  後に続いた

すべてのコンテンツフラグメントを公開した後、 `[` 2`]` パブリッシュインスタンスからコンテンツフラグメントを取得しようとすると、404 エラーが受信され、パブリッシャーからコンテンツを読み取れなくなります。
...
`[` 1`]`  aem_base_version: 6.5 aem_envId: xxxxxxx aem_envType: dev aem_program_id: xxxxxxaaem_release_id: xxxxxxxaem_service: cm-pxxxxxxxxxxxxaem_tier: ns-backend: publish/-cache <b>ブロック</b>
クラスタ： `<` クラスター`>`
code: - file_path: /var/log/aem/dispatcher.log host: publish-pxxxxxx-exxxxxxx.adobeaemcloud.com method:POST名前空間： ns-team-aem-cm-prd-nxxxxorig_time: 25/Jul/2023:15:39:11 +0000 パス： /content/cq:graphql/graphql/endpoint.json pod_name: `<` pod_name`>`
pod_uid: `<` pod_uid`>`
