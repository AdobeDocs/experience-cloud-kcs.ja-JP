...
実稼動環境で SSO ログインを有効にする SAML 2.0 設定を作成した後、Azure プロバイダーがログインを実行し、SAML トークンを取得した後、お客様がAEM側からエラーメッセージを受け取り、AEM側で認証を実行しました。<br><br> <br><br>受け取ったエラーは次のとおりです。 `[` 1`]` <br><br> <br><br><b>`[` 1`]`  \*エラー\* `[` qtp1558388453-153215`]`  com.adobe.granite.auth.saml.SamlAuthenticationHandler SAML 応答パラメーターが指定されていないか無効です。</b>
...
エラー 403 は、通常、Apache Sling Referrer Filter が有効になっていない場合に表示されます。 Apache Sling Referrer Filter の設定の確認で「Allow empty」オプションがオンになっておらず、「Allow Hosts」にホストが追加されていませんでした

SAML ログインが機能するには、ID プロバイダーのホスト名を Apache Sling Referrer Filter OSGi 設定に追加する必要があります。

これらの必須手順を完了したら、「Allow Empty」をオンにし、IdP プロバイダーを表すホストを「Allow Hosts」に追加すると、ログインが成功しました。



メモ:

IDP URL は、プロトコルを含めずに、Apache Sling Referrer Filter 設定に追加する必要があります ( 例：https://aem-sso-samlの代わりに aem-sso-saml)。
