...
- インデックスの再作成が完了せず、ループに陥った場合は、以下の「 」の節を参照してください。 *再インデックス失敗の分析*.
...
- 一連のスレッドダンプを取り、スタック内の AsyncIndexUpdate を含むスレッドを検索して、インデックス作成に要する時間の大部分がどこに費やされているか、またはインデックス作成が他のスレッドで待機しているかを確認します。 スレッドダンプを取得するには、[この記事](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17452.html?lang=ja)を参照してください。
