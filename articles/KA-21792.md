...
`CONTAINS_NOT` および *null* 値<br> <br>最初の変更は、 `CONTAINS_NOT` 演算子 以前のリリースでは、 `CONTAINS_NOT` 次を含むコンテンツフラグメントを返さなかった *null* フィルター条件が適用されたフィールドの値。 これは、次のような類似の演算子とは矛盾します。 `EQUALS_NOT` または `NOT_AT`.<br>お使いのクライアントアプリケーションが *null* AEMが `CONTAINS_NOT`.<br> <br><b>例:</b>
...
<br>モードの適用 `ALL_OR_EMPTY/ALL` および *null* 値<br>
...
<br>モードの適用： `INSTANCES`, `_instances: 0` および *null* 値<br>
...
<br>のお客様に影響を与える可能性のある改善点 `ft-sites-97` 有効<br>
...
<br>配列： `INSTANCES` と `_instances: 0` 正しく機能しない<br>
