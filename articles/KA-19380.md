...
E メール内の承認リンクをクリックすると、次のエラーが発生する場合があります。
...
オペレーターは、 [!UICONTROL キャンペーンワークフロー] フォルダーの下 [!UICONTROL 管理] - [!UICONTROL 実稼動] - [!UICONTROL 自動的に作成されたオブジェクト] パス。
