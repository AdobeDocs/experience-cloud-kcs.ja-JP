---
title: 「Adobe Targetを IMS 設定のクラウドソリューションとして選択できません」
description: 説明
...
# IMS 設定のクラウドソリューションとしてAdobe Targetを選択できません
...
AEMとAdobe Targetを統合するために、お客様がAdobe Target IMS 設定を作成しようとしています。

ただし、IMS 設定の Cloud ソリューションとして「Adobe Target」を選択すると、「証明書を取得できません。指定されたユーザー ID は既存のユーザーを識別しません」というエラーが表示され、設定を作成できません。

この問題は、Adobe Targetソリューションでのみ発生します。



再現手順:

1. ツールに移動 `>`  セキュリティ `>`  IMS の設定 `>`  作成
2. ソリューションとして Target を選択します。

UI に「証明書を取得できません。 指定されたユーザー ID は既存のユーザーを識別しません。」

ログでは、この問題に関連する唯一のエラーは次のとおりです。 `[` 1`]` .
...
ユーザーの「target-imsconfig-service」と「targetservice」の権限を確認すると、target-imsconfig-service に ACE が割り当てられていないことに気がつきました。
...
パス： /home/users/system/target-imsconfig-service

権限：allow

権限： jcr:read; rep:write
