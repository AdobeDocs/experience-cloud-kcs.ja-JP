...
description: レプリケーションキューがエラーでブロックされるAdobe Experience Manager as a Cloud Serviceの問題を修正する方法を説明します。
solution: Experience Manager as a Cloud Service
product: Experience Manager as a Cloud Service
applies-to: Experience Manager as a Cloud Service
keywords: KCS, AEMaaCS, AEM，レプリケーションキュー，ブロック，エラー，パッケージの例外， java
...
Adobe Experience Manager as a Cloud Service(AEMaaCS) のレプリケーションキューがブロックされることがあります。 この記事の手順に従って、管理者権限でこの問題を修正します。
...
AEMaaCS レプリケーションキューの 1 つが、 `javax.jcr.NamespaceException: Unknown namespace prefix: ns22_exception`.
...
1. golden-publish キューのブロックを解除します。 次に移動： <b>AEM Start</b> `>`  <b>ツール</b> `>`  <b>導入</b> `>`  <b>配布 UI</b> 「キュー」テーブルの行を太字でクリックします。


2. 次のページで、ブロック項目（ほとんどの場合はアセット）のパスを特定して記録し、「 」を選択して、その項目をキューから削除します。 これにより、最終的に残りのアクティベーションを続行できます。


3. 前の手順で記録したパスにあるアセットに移動し、を選択します。 <b>公開を管理</b>. 選択 <b>*非公開</b>* および <b>*スケジュール：今すぐ</b>*&#x200B;を選択し、次に <b>次へ</b>.


4. 上のアセットのみを選択 <b>範囲</b> ページを開き、をクリックします。 <b>非公開</b>. 確認 <b>続行</b> （参照が識別された場合）。


5. アセットを非公開にしたことを確認したら、アセットに再度移動し、「 」を選択します。 <b>クイック公開</b>.
