...
2. <b>パーソナライズされたコンテンツを様々な期間キャッシュ</b>  — サイトにパーソナライズされたコンテンツが含まれている場合は、 [Apache Sling Dynamic Includes](https://helpx.adobe.com/experience-manager/kt/platform-repository/using/sling-dynamic-include-technical-video-setup.html) AEMアプリケーションで Ajax（ブラウザーレベルでの非同期 JavaScript および XML 呼び出し）、SSI（Web サーバーレベルでの Server Side Includes）、ESI（CDN レベルでの Edge-side Includes）を利用して、異なる期間ページの異なる部分をキャッシュします。
...
5. <b>キャッシュエラーページ</b> - <b>[DispatcherPassError 1](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-install.html#ApacheWebServer) </b>（Apache Web サーバー固有）Dispatcher キャッシュから 404s などのエラーページを提供するディレクティブ。
6. <b>GZip は、事前に圧縮されたファイルを除くすべてのファイルタイプを圧縮します </b>- Apache Web サーバーで、 [mod_deflate](https://httpd.apache.org/docs/2.4/mod/mod_deflate.html) を使用できますが、次の点を確認してください。 <b>変更：User-Agent </b>ヘッダー<b> </b>が設定されていません。  Microsoft IIS で、 [動的圧縮](https://docs.microsoft.com/en-us/iis/configuration/system.webserver/httpcompression/).
...
7. <b>有効にする [/serveStaleOnError](https://helpx.adobe.com/experience-manager/kb/ServeStaleContentOnError.html)</b> /cache 設定 — AEMインスタンスがエラーを提供する際に、古いキャッシュファイルを提供します。
...
10. <b>Cache-Control および Last-Modified 応答ヘッダーのキャッシュ</b> -<b> [/headers](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#CachingHTTPResponseHeaders)</b> HTTP 応答ヘッダーをキャッシュする設定 <b>Cache-Control</b> および <b>最終変更日 </b>( および/または <b>ETag</b> ヘッダー (AEMから送信する場合 )。  これにより、CDN およびブラウザーレベルでのキャッシュを簡素化し、最適化するのに役立ちます。  これらのヘッダーをキャッシュすると、Web サーバー自体ではなく、AEMがヘッダーのみを設定するようになります。  この場合、AEMアプリケーションからヘッダーの送信を開始する必要があります。
...
Dispatcher バージョン 4.1.11 以降、 [/enableTTL 1](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html#ConfiguringTimeBasedCacheInvalidationenableTTL) は.any ファイル設定で設定できます。  この設定により、Dispatcher は HTTP Cache-Control 応答ヘッダーで設定されたキャッシュ有効期限を考慮します。  つまり、Dispatcher は CDN と同様に機能します。この CDN では、ファイルの有効期限が切れたときにキャッシュの無効化のプライマリ形式が発生します。  これを実装し、送信を開始したら、 <b>Cache-Control:max-age=... </b>AEMからのすべての応答に対して、パブリッシュインスタンスで dispatcher フラッシュエージェントを安全に無効にできます。
...
Dispatcher は、 Cache-Control ヘッダーを使用して、キャッシュファイルの無効化を制御するようになります。  その場合、Dispatcher をパブリッシュインスタンスからフラッシュする必要はなくなりました。
...
手順 2 でファイルが削除されるので、次回ユーザーが/content/foo.htmlや/content/foo.jsonのようなファイルを要求したときに、ファイルが「再取得」されると、同じファイルに対する以降の要求も、ファイルがキャッシュされるまでパブリッシュインスタンスに送信されます。  応答が遅い場合や、ホームページなどの大量のトラフィックページの場合、パブリッシュインスタンス層のフラッディングが発生する可能性があります。
