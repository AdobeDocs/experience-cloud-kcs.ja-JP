...
お客様の 1 人が、ステージパブリッシャーが正常に機能せず、このインスタンスのコンテンツを公開できず、次のエラーがエラーログに表示されるおそれがあると苦情を言っています。
...
ログ内のエラーはキャッシュに関連していませんが、サービス・ユーザーがオーサーまたはパブリッシャーのパス /var/discovery に対する適切な権限を持っていない場合に表示されます。

さらに調査したところ、お客様に/var/discovery パスの権限を確認するように依頼したところ、ステージパブリッシャーには/var の下に「discovery」フォルダーがなく、欠落しているコンテンツも多くあることがわかりました。

実稼動パブリッシャーの/var フォルダーには 20 を超える子があるのに対して、ステージパブリッシャーの/var フォルダーには 2 つだけ（「レプリケーション」と「sling」）がありました。
