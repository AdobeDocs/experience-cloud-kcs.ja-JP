...
keywords: KCS、gpg キー txt サブドメイン サブドメイン IP ipwhitelisting ipallowlisting ip 許可リストへのデータベースワークフローの委任 cname csr ssl sftp txt url 権限の監視スループット
...
を使用してコントロールパネルの機能にアクセスする手順は、クイックアクセス/experience cloud ホームに移動して、必要なアクセス権限を付与します。
...
Adobe CampaignCampaign コントロールパネルを使用すると、Adobe Campaign管理者は、主要なアセットを監視したり、IP 許可リスト、インスタンス別の SFTP ストレージ、GPG キーまたはサブドメインと証明書の管理などの管理タスクを実行したりできます。 すべての Campaign インスタンスのアセット、ワークフロー、データは、データベースに保存されます。 データベースが最大容量に達すると、インスタンスに問題が発生する可能性があります（ログインできない、メールを送信できないなど）。 したがって、最適なパフォーマンスを確保するには、インスタンスのデータベースを定期的に監視することが不可欠です。
...
- 「キャンペーン」をクリックし、「Campaign コントロールパネル」をクリックします。


Campaign コントロールパネルにアクセスできない場合は、製品管理者権限がプロファイルにない可能性があります。 必要なアクセス権を取得するには、組織内の既存の管理者に問い合わせてください。
...
<b>注意</b>  – 上記の機能は、Adobeの完全ホスト環境で使用できます。 これらの機能は、MID と RT がAdobeでホストされているハイブリッドホストのお客様に対して、まもなくリリースされます。 現時点では、Adobeハイブリッドのお客様は、IP 許可リストを実行し、Adobeにデリゲートされたサブドメインを表示できます。
