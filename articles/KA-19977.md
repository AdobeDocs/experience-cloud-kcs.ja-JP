---
title: Dispatcher デプロイメントの非アクティブ化
description: 説明
...
# の非アクティブ化 [!DNL Dispatcher] 導入

## 説明


デプロイメントのパイプラインの実行中に、次のエラーが発生します。
...
コメントアウトするか、 [!DNL Dispatcher] メインからのモジュール `pom.xml` ファイルに含めることができます（以下を参照）。 次に、パイプラインを再試行すると、 [!DNL Dispatcher] パブリッシャーとオーサーに対してAEMコードベースのみを変更し、デプロイします。
