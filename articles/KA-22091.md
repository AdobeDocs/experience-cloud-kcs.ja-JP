---
title: "パイプラインが「end_aem_createsegmentstore プロパティが見つかりません」というエラーで失敗する"
description: 説明
...
# 「end_aem_createsegmentstore プロパティが見つかりません」というエラーでパイプラインが失敗する

## 説明 {#description}

「開発へのデプロイ」ステージで、CI/CD パイプラインが失敗し続けます。 <br><br>コードベースが原因で、デプロイログにエラーが表示されませんでしたが、次のエラーが発生しています。<br><br> <br><br> <br><br> <br><br>`[` 1`]`  
...
パイプラインをチェックすると、次のエラー「errorCode」で失敗することがわかります。&quot;BLOCKED_QUEUE&quot;です。

つまり、追加の可変コンテンツのインストールを妨げて、レプリケーションキューがブロックされます。

ログで、エラーを確認します。 `[` 1`]`  がデプロイメント期間中に存在する。 ノードの可能性が最も高い — `[` 2`]`  パブリッシャーまたはプレビューサーバーにが存在せず、レプリケーションが失敗する。
