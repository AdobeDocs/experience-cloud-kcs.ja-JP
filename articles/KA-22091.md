---
title: 'パイプラインエラー： "end_aem_createsegmentstore プロパティが見つかりません"'
description: エラーが発生する問題の修正方法を説明します。Adobe Experience Managerで end_aem_createsegmentstore プロパティが見つかりません。
...
# パイプラインエラー：「end_aem_createsegmentstore プロパティが見つかりません」


「パイプラインでエラーが発生した場合： `The end_aem_createsegmentstore property is missing`、この記事の指示に従って問題を修正します。」
...
The <b>継続的統合</b> および <b>継続的な導入</b> <b>(CI/CD)</b> パイプラインは、 *開発にデプロイ* ステージ。
<br>コードベースが原因で、デプロイログにエラーが表示されませんでしたが、次のエラーが発生しています。<br> <br>`[` 1`]`  
...
パイプラインをチェックすると、エラーで失敗したことを確認できます *errorCode*: *BLOCKED_QUEUE*.

つまり、レプリケーションキューがブロックされ、追加の可変コンテンツのインストールが妨げられます。

ログで、エラーを確認します。 <b>`[` 1`]` </b> がデプロイメント期間中に存在する。 ノードの可能性が最も高い — <b>`[` 2`]` </b> パブリッシャーまたはプレビューサーバーにが存在せず、レプリケーションが失敗する。

レプリケーションキューをクリアした後、パイプラインのデプロイメントは成功しました。
